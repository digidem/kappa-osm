...
var OSM_TYPES = ['node', 'way', 'relation', 'changeset']

// OsmElement -> [Error]
function checkElement (element, op) {
  if (op === 'del') {
    if (OSM_TYPES.indexOf(element.type) !== -1 && !element.changeset) {
      return [new Error('Deleted OSM elements must have a "changeset" set.')]
    } else {
      return []
    }
  } else {
