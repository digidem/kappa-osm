            ...
            core.api.refs.get(doc.id, function (err, refs) {
              ...
              refs.forEach(function (ref) {
                ...
                rfeed.get(rseq, function (err, rdoc) {
                  ...
                  stream.push(rdoc)
                  if (rdoc.element && Array.isArray(rdoc.element.refs)) {
                    pending += rdoc.element.refs.length
                    rdoc.element.refs.forEach(addRef)
                  } else if (rdoc.element && Array.isArray(rdoc.element.members)) {
                    pending += rdoc.element.memebers.length
                    rdoc.element.members.forEach(addMember)
                  }
                  if (--pending === 0) next()
                })
              })
